...
const App: FC = () => {
    ...
    const handleInactivateEmployee = async (employeeId: number) => {
        setIsLoading(true);
        try {
            const inactivatedEmployee = await api.inactivateEmployee(employeeId);
            setEmployees(prev => prev.map(e => e.id === employeeId ? inactivatedEmployee : e));
            addNotification({ userId: currentUser.id, message: `O funcionário ${inactivatedEmployee.name} foi inativado com sucesso.` });
        } catch (error) {
    ...
    const renderView = () => {
        switch (currentView) {
            ...
            case 'client-dashboard': return <ClientDashboardView currentUser={currentUser} clients={clients} invoices={invoices} documents={documents} tasks={tasks} employees={employees} viewingClientId={viewingClientId} setCurrentView={setCurrentView} />;
