...
const Chatbot: React.FC<ChatbotProps> = ({ currentUser, tasks, documents, invoices, activeClientId }) => {
  ...
  const handleSendMessage = useCallback(async () => {
    if (userInput.trim() === '' || isLoading || !currentUser || activeClientId === null) return;
    ...
    try {
      const response = await sendBotMessage(currentInput, context);
  ...
  }, [userInput, isLoading, messages, currentUser, invoices, tasks, documents, activeClientId]);
      ...
      <footer className="p-4 border-t border-gray-200">
        <div className="flex items-center">
          <input
            ...
            disabled={isLoading}
          />
          <button onClick={handleSendMessage} disabled={isLoading || !userInput.trim()} className="ml-2 bg-primary text-white p-2 rounded-lg hover:bg-primary-dark disabled:bg-gray-400">
