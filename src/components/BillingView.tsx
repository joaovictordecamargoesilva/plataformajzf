...
import { Invoice, User, Client, Settings, AppNotification } from '../types';
...
const BillingView: React.FC<BillingViewProps> = ({ invoices, setInvoices, currentUser, clients, users, settings, addNotification, activeClientId, setIsLoading }) => {
  ...
  const downloadBoleto = (invoice: Invoice) => {
    ...
    if (invoice.boletoPdf) {
      ...
      if (client) {
          const invData = { clientId: invoice.clientId, description: invoice.description, amount: invoice.amount, dueDate: invoice.dueDate, isRecurring: invoice.isRecurring };
