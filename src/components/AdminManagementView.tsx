          ...
          {admin?.role !== 'AdminGeral' && (
            <div className="border-t pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {(Object.keys(formData.permissions) as Array<keyof UserPermissions>).map(key => {
                    ...
                    return (
                        <label key={key} className="flex items-center p-2 rounded-md hover:bg-gray-100 cursor-pointer">
                          <input type="checkbox" name={key} checked={formData.permissions[key]} onChange={handlePermissionChange} className="h-4 w-4 rounded text-primary focus:ring-primary border-gray-300"/>
                          <span className="ml-2 text-sm text-gray-700">{permissionLabels[key]}</span>
                        </label>
                    )
                })}
              </div>
