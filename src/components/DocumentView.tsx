...
import { Document, User, Client, AppNotification, DocumentStatus, DocumentTemplate, DocumentTemplateField, RequiredSignatory, Signature, Task, Employee, DocumentCategory } from '../types';
...
const generateAvisoPrevioPdf = (docData: Document, client: Client) => {
    ...
    doc.text(`À`, 20, 40);
    doc.text(`Sr(a). ${employeeName || '[Nome do Funcionário]'}`, 20, 45);
...
`;
    ...
    doc.text(employeeName || '[Nome do Funcionário]', 20, 175);
    doc.text('Ciente em: _____/_____/______', 20, 180);

    doc.save(`aviso_previo_${employeeName?.replace(/ /g, '_') || 'funcionario'}.pdf`);
};
...
const DocumentView: React.FC<DocumentViewProps> = ({ documents, setDocuments, currentUser, clients, users, addNotification, documentTemplates, directAction, setDirectAction, setTasks, activeClientId, setIsLoading, employees, handleInactivateEmployee }) => {
    ...
    try {
        ...
        if (isClient) {
            users.filter(u => u.role.includes('Admin')).forEach(admin => {
                addNotification({
                    userId: admin.id,
                    message: `${currentUser.name} solicitou um documento: ${newDoc.name}`
                });
            });
        } else {
            ...
            if(clientUser) {
                addNotification({
                    ...
                    message: `Nova solicitação de documento: ${data.requestText}`
                });
            }
        }
    } catch (error) {
  ...
  const DocumentRequestModal: React.FC<{isOpen: boolean, onClose: () => void}> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        handleSaveRequest(data as { clientId?: number; requestText: string });
        setRequestText('');
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} size="lg">
             <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="requestText" value={requestText} onChange={e => setRequestText(e.target.value)} placeholder={isClient ? "Ex: Preciso de uma cópia do meu contrato social." : "Ex: Documentos para fechamento fiscal de Fevereiro."} className="w-full p-2 border rounded mt-1" rows={3} required/>
                </div>
                <div className="mt-6 flex justify-end space-x-3">
                    <button type="button" onClick={onClose} className="bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg">Cancelar</button>
   ...
   const DocumentDetailsModal: React.FC<{
    ...
    const canApprove = !isClient && doc.status === 'AguardandoAprovacao' && doc.templateId === 'rescisao-contrato';
    const canGenerateAviso = (isClient || !isClient) && doc.templateId === 'rescisao-contrato' && (doc.status === 'Recebido' || doc.status === 'Concluido' || doc.status === 'AguardandoAprovacao' || doc.status === 'PendenteEtapa2');
        ...
        <Modal isOpen={isOpen} onClose={onClose} size="lg">
            <div>
                 ...
                 <div className="space-y-2 text-sm mb-4 border-b pb-4">
                    ...
                    <p className="text-black"><strong>Status:</strong> {formatDocumentStatus(doc.status)}</p>
                    ...
                    {doc.description && <p className="text-black mt-2"><strong>Descrição:</strong> {doc.description}</p>}
                 </div>
  ...
  const AdminView: React.FC = () => {
    const getStatusClass = (status: DocumentStatus) => {
        const classes: Partial<Record<DocumentStatus, string>> = {
            ...
            'AguardandoAprovacao': 'bg-purple-100 text-purple-800',
            'AguardandoAssinatura': 'bg-purple-100 text-purple-800',
                            ...
                            <tr key={doc.id} className="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td className="px-5 py-5 text-sm">
                                    <span className={`relative inline-block px-3 py-1 font-semibold leading-tight rounded-full ${getStatusClass(doc.status)}`}>
                                        <span className="relative">{formatDocumentStatus(doc.status)}</span>
                                    </span>
                                </td>
                                <td className="px-5 py-5 text-sm">
  ...
  const ClientView: React.FC = () => {
    ...
    const clientActivity = filteredDocuments.filter(d => d.source === 'cliente' && d.status !== 'PendenteEtapa2' && d.status !== 'AguardandoAprovacao' && d.status !== 'AguardandoAssinatura');
    const inProgressDocs = filteredDocuments.filter(d => d.status === 'PendenteEtapa2' || d.status === 'AguardandoAprovacao' || d.status === 'AguardandoAssinatura');
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <DocumentList 
                ...
                docs={inProgressDocs} 
                onDocClick={(doc) => doc.status === 'PendenteEtapa2' ? handleOpenForEditing(doc) : handleOpenDetails(doc)}
  ...
  const DocumentTemplateForm: React.FC<{ template: DocumentTemplate, onSave: (data: any) => void, onCancel: () => void, initialData?: Record<string, any>, initialStep?: number, client?: Client }> = ({ template, onSave, onCancel, initialData = {}, initialStep = 1, client }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      const fieldsForCurrentStep = template.fields ? (template.steps ? template.fields.filter(f => f.step === currentStep) : template.fields) : [];
      ...
      for (const field of fieldsForCurrentStep) {
          ...
          if (isFieldRequired && !formData[field.id] && field.type !== 'checkbox' && field.type !== 'file') {
      ...
      if (isIntermediateSubmitStep && client) {
          ...
          generateAvisoPrevioPdf(tempDocDataForPdf, client);
      }
    ...
    const fieldsForStep = template.fields ? (template.steps ? template.fields.filter(f => f.step === currentStep) : template.fields) : [];
  ...
  const DocumentTemplateModal: React.FC<{isOpen: boolean, onClose: () => void, editingDoc: Document | null}> = ({ isOpen, onClose, editingDoc }) => {
    const [step, setStep] = useState<'categories' | 'list' | 'form'>('categories');
    ...
    const categories: DocumentCategory[] = ['RH', 'Fiscal', 'Contábil', 'Societário', 'Outro'];
    const categoryIcons: Record<DocumentCategory, string> = {
        ...
        'Outro': 'M4 6h16M4 12h16M4 18h16',
    };

    const templatesByCategory = useMemo(() => {
        const grouped: Record<DocumentCategory, DocumentTemplate[]> = { 'RH': [], 'Fiscal': [], 'Contábil': [], 'Societário': [], 'Outro': [] };
        documentTemplates.forEach(t => {
            const category = t.category && categories.includes(t.category) ? t.category : 'Outro';
            grouped[category].push(t);
        });
        for (const cat in grouped) {
            (grouped[cat as DocumentCategory] as DocumentTemplate[]).sort((a, b) => a.name.localeCompare(b.name));
        }
        return grouped;
    }, [documentTemplates]);
    ...
    const handleSave = async (data: { formData: Record<string, any>, file?: { name: string, type: string, content: string } }) => {
        ...
        try {
            if (editingDoc) { // This is an update (e.g., submitting Step 2 of rescission)
                ...
                const notificationMessage = newDoc.status === 'AguardandoAprovacao'
    ...
    const filteredTemplates = selectedCategory ? templatesByCategory[selectedCategory].filter(t => t.name.toLowerCase().includes(searchTerm.toLowerCase())) : [];

    return (
        <Modal isOpen={isOpen} onClose={handleClose} size="xl">
            {step === 'categories' && !editingDoc && !taskToComplete && (
                 <>
                    <h3 className="text-xl font-bold text-black mb-4">Enviar Novo Documento</h3>
                    <p className="text-gray-600 mb-6">Selecione a categoria do documento que deseja enviar.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {categories.filter(cat => templatesByCategory[cat].length > 0).map(cat => (
                            <button key={cat} onClick={() => { setSelectedCategory(cat); setStep('list'); }} className="flex flex-col items-center justify-center p-6 bg-gray-50 hover:bg-primary-dark/10 rounded-lg transition-colors border">
             ...
             {step === 'list' && selectedCategory && (
                 <>
                    <div className="flex items-center mb-4">
                        <button onClick={() => setStep('categories')} className="text-primary p-2 rounded-full hover:bg-gray-100 mr-2">
                            <Icon path="M10 19l-7-7m0 0l7-7m-7 7h18" className="w-6 h-6"/>
                        </button>
                        <h3 className="text-xl font-bold text-black">Enviar Documento: {selectedCategory}</h3>
                    </div>
                    <input type="text" placeholder="Buscar na lista..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full p-2 pl-10 border rounded-lg mb-4" />
                    <ul className="space-y-2 max-h-96 overflow-y-auto">
                        {filteredTemplates.map(template => (
                            <li key={template.id}>
                                <button onClick={() => { setSelectedTemplateId(template.id); setStep('form'); }} className="w-full text-left p-3 bg-gray-50 hover:bg-gray-100 rounded-md transition-colors text-sm text-gray-800">
                                    {template.name}
                                </button>
                            </li>
                        ))}
            ...
            {(step === 'form' && selectedTemplate) && (
                 <div className="mt-6 border-t pt-6">
                    <DocumentTemplateForm template={selectedTemplate} onSave={handleSave} onCancel={handleClose} initialData={editingDoc?.formData} initialStep={editingDoc?.workflow?.currentStep} client={clientForForm} />
                 </div>
            )}
        </Modal>
    );
  ...
  return (
    <div>
        ...
        <div className="flex items-center space-x-2">
           {isClient ? (
             <>
                ...
                <button onClick={() => setTemplateModalOpen(true)} className="flex items-center bg-primary text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-primary-dark transition-colors">
                    <Icon path="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" className="w-5 h-5 mr-2" />
      ...
      <DocumentRequestSelectionModal isOpen={isRequestSelectionModalOpen} onClose={() => setRequestSelectionModalOpen(false)} onSelect={(docName, observation) => handleSaveRequest({ requestText: docName, description: observation })} />
