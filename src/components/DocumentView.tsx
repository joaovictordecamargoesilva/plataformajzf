...
const DocumentView: React.FC<DocumentViewProps> = ({ documents, setDocuments, currentUser, clients, users, addNotification, documentTemplates, directAction, setDirectAction, setTasks, activeClientId, setIsLoading, employees, handleInactivateEmployee }) => {
  ...
  const handleSelection = (docName: string, observation: string) => {
    ...
    if (isSelectionModalOpen.type === 'send') {
        ...
        handleSaveRequest({ requestText: docName, description: observation });
    }
  };
...
const handleUpdateStatus = async (docId: number, status: DocumentStatus) => {
                    ...
                    {filteredDocuments.map(doc => {
                        ...
                        return (
                            <tr key={doc.id} className="border-b border-gray-200 hover:bg-gray-50">
                                ...
                                <td className="px-5 py-5 text-sm text-black">{doc.source === 'cliente' ? 'Cliente' : 'Escritório'}</td>
                                <td className="px-5 py-5 text-sm">
  ...
  const ClientView: React.FC = () => {
    const pendingRequestsFromOffice = filteredDocuments.filter(d => d.source === 'escritorio' && d.status === 'AguardandoAssinatura');
    const clientActivity = filteredDocuments.filter(d => !pendingRequestsFromOffice.includes(d) && d.status !== 'PendenteEtapa2' && d.status !== 'AguardandoAprovacao');
    const inProgressDocs = filteredDocuments.filter(d => d.status === 'PendenteEtapa2' || d.status === 'AguardandoAprovacao');
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <DocumentList 
                    title="Aguardando sua Assinatura" 
                    docs={pendingRequestsFromOffice} 
                    onDocClick={handleOpenDetails}
                    emptyText="Nenhum documento para assinar."
