...
import { Document, User, Client, AppNotification, DocumentStatus, DocumentTemplate, DocumentTemplateField, RequiredSignatory, Signature, Task } from '../types';
...
// --- Main Component ---
const DocumentView: React.FC<DocumentViewProps> = ({ documents, setDocuments, currentUser, clients, users, addNotification, documentTemplates, directAction, setDirectAction, setTasks, activeClientId, setIsLoading }) => {
  ...
  const handleSaveRequest = async (data: { clientId?: number; requestText: string }) => {
    setIsLoading(true);
    try {
        const targetClientId = isClient ? activeClientId! : data.clientId!;
        const newDoc = await api.createDocumentRequest(targetClientId, data.requestText, currentUser.name, isClient ? 'cliente' : 'escritorio');
  ...
  const handleSendSelection = (docName: string) => {
    ...
    const formDocMapping: { [key: string]: string } = {
        ...
        // The list name is longer than the template name, so find is better
    };
  ...
  const handleSimpleSend = async (file: File) => {
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
            <form onSubmit={handleSubmit}>
                ...
                <div className="space-y-4">
                    <p className="text-gray-600">Por favor, anexe o arquivo correspondente a "{data.name}".</p>
                    <div>
                        <label htmlFor="simple-file-upload" className="block text-sm font-medium text-gray-700">Arquivo</label>
                        <input id="simple-file-upload" type="file" onChange={e => setFile(e.target.files?.[0] || null)} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-dark/10 file:text-primary hover:file:bg-primary-dark/20 mt-1" required/>
                    </div>
                </div>
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} size="xl">
            ...
            <div className="mt-6 border-t pt-6">
                {selectedTemplate && <DocumentTemplateForm template={selectedTemplate} onSave={handleSave} onCancel={handleClose} initialData={editingDoc?.formData} initialStep={editingDoc?.workflow?.currentStep} client={clientForForm} />}
            </div>
        </Modal>
    );
  };
  ...
  return (
    <div>
      ...
      <DocumentRequestSelectionModal isOpen={isRequestSelectionModalOpen} onClose={() => setRequestSelectionModalOpen(false)} onSelect={(docName) => handleSaveRequest({ requestText: docName })} />
