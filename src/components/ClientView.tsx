...
const ClientForm: React.FC<{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSaving(true);
        // The payload now needs to include userId for password updates
        ...
        <table className="min-w-full leading-normal">
          ...
          <tbody>
            {filteredClients.map(client => {
              const responsibleUser = users.find(u => u.clientIds.includes(client.id));
              return (
              <tr key={client.id} className="border-b border-gray-200 hover:bg-gray-50">
                <td className="px-5 py-5 text-sm">
                  <p className="text-gray-900 font-semibold whitespace-no-wrap">{client.company}</p>
                  <p className="text-gray-700 whitespace-no-wrap">{responsibleUser?.name || client.name}</p>
                </td>
                <td className="px-5 py-5 text-sm">
