...
const SideNav: React.FC<SideNavProps> = ({ currentView, setCurrentView, currentUser, onOpenQuickSend }) => {
  const allNavItems = [
    ...
    { id: 'relatorios', label: 'Análise & Oportunidades', icon: 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z', permissionKey: 'canViewReports' },
  ...
  const getVisibleNavItems = () => {
      if(currentUser.role === 'Cliente') {
          const clientViews = ['dashboard', 'documentos', 'cobranca', 'tarefas', 'ponto', 'relatorios', 'novos-relatorios', 'simulacoes', 'quicksend'];
          // A little re-ordering for clients
          const orderedIds = ['dashboard', 'documentos', 'cobranca', 'tarefas', 'ponto', 'quicksend', 'novos-relatorios', 'simulacoes', 'relatorios'];
          return allNavItems.filter(item => clientViews.includes(item.id)).sort((a,b) => orderedIds.indexOf(a.id) - orderedIds.indexOf(b.id));
      }
      
      // For Admins, filter out 'relatorios' (Busca Rapida) and action items
      const adminItems = allNavItems.filter(item => item.id !== 'relatorios' && !item.isAction);
      ...
      if(currentUser.role === 'AdminLimitado' && currentUser.permissions){
          return adminItems.filter(item => {
              // 'novos-relatorios' and 'simulacoes' uses 'canViewReports' permission.
              if (item.id === 'novos-relatorios' || item.id === 'simulacoes') {
                 return currentUser.permissions['canViewReports'];
              }
              // 'ponto' uses 'canManageTasks' permission
              if (item.id === 'ponto') {
                 return currentUser.permissions['canManageTasks'];
              }
  ...
  return (
    <div className="w-64 bg-primary-dark text-white flex flex-col flex-shrink-0">
      <div className="flex items-center justify-center h-20 border-b border-primary">
        <h1 className="text-2xl font-bold text-white tracking-wider">JZF</h1>
      </div>
