...
// Documents
export const createDocumentRequest = async (clientId: number, requestText: string, uploadedBy: string, source: 'cliente' | 'escritorio'): Promise<Document> => {
     const response = await fetch(`${API_BASE_URL}/documents/request`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId, requestText, uploadedBy, source })
    });
    return handleResponse(response);
}
