...
const NewReportsView: React.FC<NewReportsViewProps> = ({ currentUser, clients, invoices, documents, tasks, activeClientId }) => {
    ...
    const AdminReport = () => {
        ...
        const getClientMetrics = (clientId: number) => {
            ...
            return {
                ...
                pendingDocuments: clientDocs.filter(doc => doc.status === 'Pendente' || doc.status === 'Pendente Etapa 2').length,
    ...
    const ClientReport = () => {
        ...
        const completedTasks = tasks.filter(task => task.clientId === activeClientId && task.status === 'Concluída').length;
