...
const PontoView: React.FC<PontoViewProps> = ({ clients, employees, setEmployees, timeSheets, setTimeSheets, currentUser, addNotification, users, activeClientId, setIsLoading }) => {
    ...
    const [selectedClientForAdmin, setSelectedClientForAdmin] = useState<Client | null>(!isClient ? (clients.find(c => c.status === 'Ativo') || null) : null);
    const [modalState, setModalState] = useState<{ type: null | 'addEmployee' | 'addTimeSheet' | 'editEmployee' | 'import'; data?: any }>({ type: null });
    ...
    const handleSaveTimeSheet = async (timeSheetData: Omit<TimeSheet, 'id'>) => {
        ...
        try {
            ...
            if (isClient && timeSheetData.status === 'Enviado para Análise') {
    ...
    const handleImportSheet = (file: File) => {
        ...
        alert("Funcionalidade de importação de planilha em desenvolvimento. O arquivo foi recebido no console.");
        setModalState({ type: null });
    };

    const EmployeeModal: React.FC<{ clientName: string, employee?: Employee, onSave: (data: any, id: number | null) => void, onCancel: () => void, initialData?: any }> = 
    ({ clientName, employee, onSave, onCancel, initialData }) => {
        const [formData, setFormData] = useState({
            name: employee?.name || initialData?.name || '',
            role: employee?.role || initialData?.role || '',
            salary: employee?.salary || initialData?.salary || 0,
        });
    ...
    const EmployeeImportModal: React.FC<{ onImport: (file: File) => void, onCancel: () => void }> =
    ({ onImport, onCancel }) => {
        const [file, setFile] = useState<File | null>(null);

        return (
             <div>
                <h3 className="text-xl font-semibold mb-2 text-black">Importar Funcionários via Planilha</h3>
                <p className="text-gray-600 mb-4">Selecione um arquivo .xlsx ou .csv com as colunas: Nome, Cargo, Salário.</p>
                <input type="file" onChange={(e) => setFile(e.target.files?.[0] || null)} accept=".xlsx, .csv" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-dark/10 file:text-primary hover:file:bg-primary-dark/20" />
                <div className="mt-6 flex justify-end space-x-3">
                    <button type="button" onClick={onCancel} className="bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded-lg">Cancelar</button>
                    <button type="button" onClick={() => file && onImport(file)} disabled={!file} className="bg-primary text-white font-bold py-2 px-4 rounded-lg disabled:opacity-50">Importar</button>
                </div>
            </div>
        )
    }
    ...
    ({ employee, existingSheet, onSave, onCancel }) => {
        ...
        const handleSubmit = () => {
             onSave({
                ...
                status: isClient ? 'Enviado para Análise' : 'Processado'
             });
        };
    ...
    const getStatusBadge = (sheet: TimeSheet | undefined) => {
        ...
        switch(sheet.status) {
            case 'Enviado para Análise': return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800">Enviado para Análise</span>;
            ...
            case 'Erro na Análise': return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">Erro na Análise</span>;
    ...
    const renderEmployeeTable = (employeeList: Employee[], title: string) => (
        <div className="overflow-x-auto">
            {title && <h4 className="text-lg font-semibold text-gray-700 mt-6 mb-2">{title}</h4>}
    ...
    const activeEmployees = clientForView ? employees.filter(e => e.clientId === clientForView.id && e.status === 'Ativo') : [];
    const inactiveEmployees = clientForView ? employees.filter(e => e.clientId === clientForView.id && e.status === 'Inativo') : [];
    ...
    return (
        <div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                             <button onClick={() => setModalState({ type: 'import' })} className="flex items-center bg-green-600 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-green-700 transition-colors text-sm">
                                <Icon path="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" className="w-4 h-4 mr-2" />
                                Importar via Planilha
                            </button>
                            <button onClick={() => setModalState({ type: 'addEmployee' })} className="flex items-center bg-primary text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-primary-dark transition-colors text-sm">
                    ...
                    {inactiveEmployees.length > 0 && (
                        <div className="mt-6">
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" checked={showInactive} onChange={() => setShowInactive(!showInactive)} className="h-4 w-4 rounded text-primary focus:ring-primary border-gray-300" />
                                <span className="text-sm font-medium text-gray-700">Mostrar funcionários inativos</span>
                            </label>
                            {showInactive && renderEmployeeTable(inactiveEmployees, 'Funcionários Inativos')}
                        </div>
                    )}
                </div>
            ...
            <Modal isOpen={modalState.type === 'addEmployee' || modalState.type === 'editEmployee'} onClose={() => setModalState({ type: null })}>
                {clientForView && <EmployeeModal clientName={clientForView.name} employee={modalState.type === 'editEmployee' ? modalState.data : undefined} onSave={handleSaveEmployee} onCancel={() => setModalState({ type: null })} initialData={modalState.type === 'addEmployee' ? modalState.data : undefined}/>}
            </Modal>
             <Modal isOpen={modalState.type === 'import'} onClose={() => setModalState({ type: null })}>
                <EmployeeImportModal onImport={handleImportSheet} onCancel={() => setModalState({ type: null })}/>
            </Modal>
