...
const TasksView: React.FC<TasksViewProps> = ({ tasks, setTasks, currentUser, clients, users, addNotification, setDirectAction, setCurrentView, activeClientId, setIsLoading }) => {
  ...
  const handleCompleteTaskByButton = async (task: Task) => {
    setIsLoading(true);
    try {
        const updatedTask = await api.updateTaskStatus(task.id, 'Concluída');
                  ...
                  <li key={task.id} className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center text-xs text-gray-500 mt-1">
                          <span className={`px-2 py-1 font-semibold leading-tight rounded-full ${getStatusClass(task.status)}`}>{task.status}</span>
                    ...
                    <button onClick={() => setEditingTask(task)} className="text-gray-400 hover:text-primary-dark">
