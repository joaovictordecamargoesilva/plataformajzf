    ...
    const ComplianceCard: React.FC<{finding: ComplianceFinding}> = ({ finding }) => {
        const getStatusInfo = (status: ComplianceFinding['status']) => {
            switch(status) {
                ...
                case 'Atenção': return { badge: 'bg-yellow-100 text-yellow-800', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z', iconColor: 'text-yellow-500' };
                case 'Pendência': return { badge: 'bg-red-100 text-red-800', icon: 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636', iconColor: 'text-red-500' };
    ...
    return (
        <div>
            <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
                <h2 className="text-3xl font-bold text-black mb-4 md:mb-0">Análise e Oportunidades</h2>
            ...
            {activeClientId ? (
                     ...
                     <div className="space-y-4">
                        ...
                        {clientComplianceFindings.length > 0 ? (
                           clientComplianceFindings.map(finding => <ComplianceCard key={finding.id} finding={finding} />)
                            ...
                            <div className="text-center py-10 bg-white rounded-lg shadow-lg">
                                <Icon path="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" className="w-12 h-12 mx-auto text-gray-400 mb-4" />
                                <h3 className="text-lg font-semibold text-black">Nenhum resultado de verificação</h3>
                                <p className="text-text-secondary">Clique em "Verificar Pendências" para iniciar uma nova busca.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="mt-12">
                     <h3 className="text-2xl font-bold text-black mb-4">Busca de Oportunidades</h3>
                     <div className="space-y-4">
                        {clientOpportunities.length > 0 ? (
                            clientOpportunities.map(opp => {
                                const deadlineInfo = getDeadlineInfo(opp.submissionDeadline);
                                return (
                                    <div key={opp.id} className="bg-white p-6 rounded-lg shadow-lg">
                                        <div className="flex justify-between items-start">
                                            <h3 className="text-xl font-bold text-black">{opp.title}</h3>
                                            <span className={`px-3 py-1 text-sm font-semibold rounded-full ${opp.type === 'Incentivo Fiscal' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                                                {opp.type}
                                            </span>
                                        </div>
                                        <p className="text-sm text-gray-500 mt-1">
                                            Encontrado em: {new Date(opp.dateFound).toLocaleDateString('pt-BR')}
                                        </p>
                                        <p className="text-text-primary my-4">{opp.description}</p>
                                        {deadlineInfo && (
                                            <p className={`font-bold text-sm mb-2 ${deadlineInfo.color}`}>
                                                {deadlineInfo.text}
                                            </p>
                                        )}
                                        <div className="border-t pt-2">
                                            <a href={opp.source.startsWith('http') ? opp.source : '#'} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline break-all">
                                                Fonte: {opp.source}
                                            </a>
                                        </div>
                                    </div>
                            ...
                            <div className="text-center py-10 bg-white rounded-lg shadow-lg">
                                <Icon path="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" className="w-12 h-12 mx-auto text-gray-400 mb-4" />
                                <h3 className="text-lg font-semibold text-black">Nenhuma oportunidade encontrada</h3>
                                <p className="text-text-secondary">Nenhuma oportunidade foi encontrada para sua empresa ainda. Tente uma nova busca.</p>
                            </div>
                        )}
                    </div>
                </div>
                 ...
                 <div className="text-center py-10 bg-white rounded-lg shadow-lg">
                    ...
                    <h3 className="text-lg font-semibold text-black">Informações da empresa não encontradas</h3>
                    <p className="text-text-secondary">Não foi possível associar sua conta a uma empresa. Por favor, entre em contato com o suporte.</p>
                </div>
            )}
        </div>
    );
