...
import { User, Client, Invoice, AppNotification, TaskTemplateSet, Employee, TimeSheet, Document, Signature, Task, TaskStatus, DocumentTemplate, Settings, Opportunity, ComplianceFinding } from './types';
...
export const updateClient = async (clientData: any): Promise<Client> => {
...
// Documents
export const createDocumentRequest = async (clientId: number, requestText: string, uploadedBy: string, source: 'cliente' | 'escritorio', description?: string, file?: any): Promise<Document> => {
     const response = await fetch(`${API_BASE_URL}/documents/request`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId, requestText, uploadedBy, source, description, file })
    });
    return handleResponse(response);
}

export const sendDocumentFromAdmin = async (data: { clientId: number; docName: string; file: File; signatoryIds: string[]; }, fileContent: string, uploadedBy: string): Promise<Document> => {
...
export const createDocumentFromTemplate = async (data: any): Promise<Document> => {
...
export const updateDocumentFromTemplate = async (docId: number, template: DocumentTemplate, data: any): Promise<Document> => {
...
export const signDocument = async (docId: number, signature: Signature, newPdfBytes: string): Promise<Document> => {
...
export const createSimpleDocument = async (docData: any): Promise<Document> => {
    const response = await fetch(`${API_BASE_URL}/documents/simple-send`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(docData)
    });
...
// --- Gemini API Calls via Backend ---

export const getClientDataFromCnpj = async (cnpj: string): Promise<any> => {
    const response = await fetch(`${API_BASE_URL}/gemini/cnpj-lookup`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const analyzeTimeSheet = async (
    ...
    const response = await fetch(`${API_BASE_URL}/gemini/analyze-timesheet`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fileContentBase64, mimeType, employee, month, year }),
    });
    return handleResponse(response);
};
...
export const sendBotMessage = async (message: string, context: string): Promise<{ reply: string }> => {
