...
const onboardClientHandler = async (req: express.Request, res: express.Response) => {
...
const updateClientHandler = async (req: express.Request, res: express.Response) => {
    if (req.user?.role !== 'AdminGeral' && !req.user?.permissions?.canManageClients) {
        return res.status(403).json({ message: 'Acesso negado.' });
    }
...
// Inactivate a client
const inactivateClientHandler = async (req: express.Request, res: express.Response) => {
     if (req.user?.role !== 'AdminGeral' && !req.user?.permissions?.canManageClients) {
        return res.status(403).json({ message: 'Acesso negado.' });
    }
...
const deleteClientHandler = async (req: express.Request, res: express.Response) => {
    if (req.user?.role !== 'AdminGeral' && !req.user?.permissions?.canManageClients) {
        return res.status(403).json({ message: 'Acesso negado.' });
    }
    const clientId = parseInt(req.params.id, 10);
