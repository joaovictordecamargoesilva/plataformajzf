...
const getInvoiceUpdates = async (req: express.Request, res: express.Response) => {
...
const updateInvoiceAmount = async (req: express.Request, res: express.Response) => {
    if (req.user?.role !== 'AdminGeral' && !req.user?.permissions?.canManageBilling) {
        return res.status(403).json({ message: 'Acesso negado.' });
    }
    const { id } = req.params;
...
const updateInvoiceStatus = async (req: express.Request, res: express.Response) => {
    if (req.user?.role !== 'AdminGeral' && !req.user?.permissions?.canManageBilling) {
        return res.status(403).json({ message: 'Acesso negado.' });
    }
    const { id } = req.params;
