...
const createEmployeesBatch = async (req: express.Request, res: express.Response) => {
...
// Inactivate employee
const inactivateEmployee = async (req: express.Request, res: express.Response) => {
    const employeeId = parseInt(req.params.id, 10);
    const updatedEmployee = await req.prisma.employee.update({
        where: { id: employeeId },
        data: { status: 'Inativo' },
...
// Save timesheet
const saveTimeSheet = async (req: express.Request, res: express.Response) => {
